...
class MultiNetworkTransferBot {
    constructor() {
        ...
        this.wallets = [];
        this.activeJobs = 0;
        this.maxConcurrentJobs = 5; // کاهش تعداد جاب‌های همزمان برای جلوگیری از خطاهای درخواست‌های RPC
        this.maxRetries = 3; // تعداد تلاش‌های مجدد برای هر عملیات
        this.retryDelay = 5000; // فاصله زمانی برای تلاش مجدد (میلی‌ثانیه)
        this.results = {};
        this.walletInfo = {};
        this.providers = {}; // کش کردن provider ها
    }
    ...
    async showMenu() {
        ...
        // جمع‌آوری اطلاعات والت‌ها برای همه شبکه‌ها قبل از شروع عملیات
        await this.gatherWalletsInfo();
        
        // نمایش اطلاعات والت‌ها برای همه شبکه‌ها
    ...
    async gatherWalletsInfo() {
        console.log(chalk.cyan(`\n🔍 Gathering information for ${this.wallets.length} wallet(s) on ${this.selectedNetworks.length} network(s)...\n`));
        
        // جمع‌آوری اطلاعات همه والت‌ها برای همه شبکه‌ها
    ...
    async displayAllWalletsInfo() {
        console.log(chalk.cyan('\n💰 Wallets Information Summary:\n'));
        
        for (const walletIndex in this.walletInfo) {
            const wallet = this.wallets.find(w => w.index === parseInt(walletIndex));
            ...
            console.log(''); // خط خالی بین والت‌ها
        }
    }
    
    async executeTransactions() {
        console.log(chalk.cyan(`\n🚀 Starting transactions on ${this.selectedNetworks.length} network(s) with ${this.wallets.length} wallet(s)\n`));
        
        const promises = [];
        
        // موازی‌سازی بر اساس شبکه و والت
        for (const networkKey of this.selectedNetworks) {
    ...
    showFinalReport() {
        ...
        // نمایش نتایج به صورت گروه‌بندی شده بر اساس والت
